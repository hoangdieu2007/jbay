...
public class UserRepository {
    ...
    // cache moved here
    private final Map<String, User> sessionCache =
    ...
    public boolean createSession(String sessionId, User user) {
        ...
        boolean success = userDAO.insertSession(
                sessionId,
                user.getId()
        );
    ...
    public List<User> getAllNormalUsers() {
        ...
        List<UserData> rawUsers =
                userDAO.getAllNormalUsers();

        List<User> users = new ArrayList<>();

        if (rawUsers != null) {
            for (UserData data : rawUsers) {
